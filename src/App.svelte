<script>
  ...
  const onLoginSuccess = (event) => {
    ...
    setTimeout(() => {
      skill =
        prompt(`What skill would you like to work on today, ${user.Name}?`) ||
        "Skill"; // Fallback to 'Skill' if input is empty
    }, 100);
  };
  ...
  {#if loggedIn}
    ...
    <section class="card date-picker-section">
      ...
      <div class="date-picker">
        ...
        <button on:click={handleJournalFetch} class="fetch-button"
          >Fetch Journal Data</button
        >
      </div>
    </section>
    ...
    <section class="card goal-section">
      ...
      <form>
        ...
        <input
          type="number"
          id="skillPracticeGoal"
          bind:value={skillPracticeGoal}
          min="1"
        />
    ...
    <!-- Additional Activity Tracking -->
    <section class="card additional-activity-section">
      <h2>Additional Activity</h2>
      <AdditionalActivity
        bind:AddYes
        bind:additionalActivityName
        bind:inputType
        bind:activityValue
      />
    </section>
    ...
    <section class="card progress-tracking-section">
      ...
      <Progress
        habitProgress={skillPracticeProgress}
        habitName="Skill Practice"
      />
